...
function isEcommerce(req, res, next) {
  // Verifica se o usuário tem a permissão de Ecommerce
  if (!req.session.user || req.session.user.role !== 'ecommerce') {
    req.session.message = 'Você não tem permissão para acessar esta página. Acesse o portal com um usuário Ecommerce.'; // Mensagem de erro
    return res.redirect('/login'); // Redireciona para a página de login
  }
  next();
}
